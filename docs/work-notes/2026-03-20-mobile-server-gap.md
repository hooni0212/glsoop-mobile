...
| `AUTHOR-P0-01` | `반영됨` | `2026-03-21` | `src/features/users/useAuthorPosts.ts`를 `offset/limit + has_more`와 `sort` 기준으로 재설계했고, `src/screens/Author.tsx`에 팔로우/정렬/소개 토글/최신 글 CTA를 반영했다. 서버 `scripts/e2e_author_posts_contract.js`와 모바일 `e2e/author-flow.spec.ts` 기준으로 `정렬 전환 / 더보기 / 재진입 refetch`까지 확인했다 | 실기기에서 한 번 더 보는 QA는 남을 수 있지만 구현/자동 검증 기준으로는 완료 |
...
| 작가 페이지/소셜 | 구현됨에 가까운 부분 구현 | 작가 프로필/글 목록, 팔로우, 정렬, 소개 토글, 최신 글 CTA, 공유, 간단한 overflow 메뉴까지 반영했고 서버/모바일 자동 검증도 확보했다. 남은 차이는 웹 쪽 세부 액션과 실기기 QA가 중심이다 |
...
| 회원가입 + 이메일 인증 | `../glsoop/public/html/signup.html`, `../glsoop/public/js/signup.js`, `../glsoop/public/js/verify-email.js` | `POST /signup`, `POST /verify-email`, `POST /verify-email/resend` | `구현됨` | 모바일 가입 화면과 OTP 단계는 반영했고, 서버 `tests/e2e/auth-signup-consent.spec.js`, `scripts/e2e_signup_outbox.js`, 모바일 `e2e/signup-flow.spec.ts` 기준으로 `signup -> OTP -> 자동 로그인`까지 검증했다 | 유지 |
| 회원가입 시 법적 동의/버전 검증 | `../glsoop/tests/e2e/auth-signup-consent.spec.js` | `GET /api/runtime-config`, `POST /api/signup` | `구현됨` | 모바일 가입 화면에 필수 동의와 version payload를 반영했고, 서버 consent E2E/ signup outbox / 모바일 signup-flow E2E 기준으로 legal fields 해피패스를 확인했다 | 유지 |
...
| 기능 | 서버 웹 구현 근거 | 서버 API 근거 | 모바일 현재 상태 | 차이 메모 | 우선순위 |
| --- | --- | --- | --- | --- | --- |
| 작가 프로필/작성 글 목록 | `../glsoop/public/html/author.html`, `../glsoop/public/js/author.js` | `GET /api/users/:id/profile`, `GET /api/users/:id/posts` | `구현됨` | 모바일 `useAuthorPosts`를 서버 `offset/has_more` 패턴으로 맞췄고, 서버 `scripts/e2e_author_posts_contract.js`, 모바일 `e2e/author-flow.spec.ts` 기준으로 `newest/oldest/likes + offset/has_more`, 정렬 전환, 더보기, 재진입 refetch까지 검증했다 | 유지 |
...
아래 항목은 현재 코드/문서/자동 검증만으로는 완전히 닫을 수 없고, 실제 기기 또는 실제 메일 앱 환경이 필요하다.

#### `AUTH-P0-02`, `AUTH-P0-03` 네이티브 로그인 유지/만료 복귀

- 목적
  - `ios/android`에서 Bearer 토큰 + secure storage가 실제로 유지되는지 확인한다.
- 사전 조건
  - 네이티브 빌드 또는 동등 환경에서 앱 실행 가능
  - 로그인 가능한 테스트 계정 준비
- 수행 절차
  - 로그인 화면에서 정상 계정으로 로그인한다.
  - 홈 진입 후 앱을 완전히 종료한다.
  - 앱을 다시 열고 로그인 상태가 유지되는지 확인한다.
  - 보호 화면(`write`, `profile-customize`, 게시글 상세 등)으로 진입한다.
  - 서버 토큰을 만료시키거나 테스트 환경에서 `/api/me`가 `401`을 반환하게 만든다.
  - 앱을 다시 foreground로 두거나 보호 화면을 재진입한다.
- 기대 결과
  - 앱 재기동 후 `/api/me` 성공과 함께 로그인 상태가 유지된다.
  - `401` 이후 저장된 토큰이 정리되고 auth 화면으로 복귀한다.
  - auth 복귀 후 로그인하면 원래 `redirect` 목적지로 돌아간다.
- 실패 시 우선 확인 경로
...
#### 비밀번호 재설정 메일 클릭 딥링크 검증

- 목적
  - 실제 메일 앱에서 앱 딥링크 `glsoopmobile://reset-password?...`가 열리는지 확인한다.
- 사전 조건
  - 메일 수신 가능한 테스트 계정 준비
  - 앱이 설치된 실기기 또는 딥링크 가능한 환경 준비
- 수행 절차
  - 로그인 화면에서 `비밀번호 찾기`로 진입한다.
  - 테스트 이메일로 재설정 요청을 보낸다.
  - 수신된 메일에서 앱 딥링크를 탭한다.
  - 앱이 열리면 reset-password 화면에서 새 비밀번호를 입력하고 저장한다.
  - 앱이 설치되지 않은 환경에서는 웹 fallback 링크도 확인한다.
- 기대 결과
  - 앱 설치 환경에서는 `reset-password` 화면으로 바로 열린다.
  - 토큰 검증 후 새 비밀번호 저장이 성공한다.
  - 완료 후 로그인 화면 또는 의도한 복귀 화면으로 이동한다.
  - 앱 미설치 환경에서는 웹 fallback 링크가 동작한다.
- 실패 시 우선 확인 경로
  - `src/screens/AuthForgotPassword.tsx`
  - `src/screens/AuthResetPassword.tsx`
  - `../glsoop/routes/authRoutes.js`
  - `../glsoop/services/mailer.js`

#### 성장/코스메틱 실사용 반영 검증

- 목적
  - 퀘스트 보상 직후 코스메틱 인벤토리와 프로필 꾸미기 화면이 실제로 최신 상태를 보여주는지 확인한다.
- 사전 조건
  - 보상 수령 가능한 퀘스트 상태 준비
- 수행 절차
  - 성장 화면에서 코스메틱 보상을 주는 퀘스트를 클레임한다.
  - 직후 `프로필 꾸미기` CTA 또는 프로필 꾸미기 화면으로 이동한다.
  - 획득한 코스메틱이 즉시 인벤토리에 보이는지 확인한다.
  - 코스메틱 적용 후 작가/마이페이지 카드 반영도 확인한다.
- 기대 결과
  - claim 직후 최신 코스메틱이 인벤토리에 보인다.
  - 적용 후 프로필 화면과 작가 화면에 같은 결과가 반영된다.
- 실패 시 우선 확인 경로
  - `src/screens/growth/Quests.tsx`
  - `src/screens/ProfileCustomize.tsx`
  - `src/features/cosmetics/useMyCosmetics.ts`
...
- `2026-03-21`: 모바일 `e2e/author-flow.spec.ts`를 추가하고 `src/screens/Author.tsx`에 정렬 칩 `testID`를 보강해 작가 화면의 `정렬 전환 -> 더보기 -> 재진입 refetch` 흐름을 실제 웹 E2E로 통과시켰다.
- `2026-03-22`: 남은 수동 작업을 `실기기/실사용 검증 체크리스트`로 정리해, 이제 직접 필요한 항목은 `네이티브 로그인 유지/만료 복귀`, `비밀번호 재설정 메일 딥링크`, `성장/코스메틱 실사용 반영` 확인으로 명확히 한정했다.
