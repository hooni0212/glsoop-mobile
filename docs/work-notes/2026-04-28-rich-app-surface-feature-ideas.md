...
### 3.0 2026-04-29 인스타형 피드 수정 방향

현재 홈/상세 피드는 인스타 기본 피드에 가깝게 유지하되, 화면에 기능 없는 버튼이나 과한 장식이 남지 않도록 정리한다.

반영 기준:

- 홈 상단의 원형 장르 링은 제거한다.
- 상단 브랜드 영역은 `글숲` 로고/이름이 더 또렷하게 보이도록 키우고, 검색창은 과하게 길지 않게 줄인다.
- 홈 피드 카드의 의미 없는 전송/공유 아이콘은 제거한다.
- 피드 카드의 작가 영역은 작가 페이지로 이동해야 한다.
- 피드 카드의 작가 마커는 기존 프로필 뱃지 시스템의 대표 뱃지를 우선 표시하고, 없으면 작성자 이니셜을 표시한다.
- 홈 피드 카드 크기는 인스타형 피드의 큰 이미지감을 유지하고, 글 상세에서 읽기 이미지를 더 크게 보여주는 방향으로 차이를 만든다.
- 글 상세는 작가/날짜/종류 태그를 맨 위에 두지 않고, 글 이미지를 먼저 크게 보여준 뒤 하단에 메타 정보를 배치한다.
- 글 상세의 상단 뒤로가기/더보기 버튼은 별도 바 공간을 차지하지 않고 이미지 위에 오버레이한다.
- 글 이미지를 감싸는 회색 둥근 프레임은 제거하고, 그 공간까지 이미지를 크게 확장한다.
- 긴 글은 이미지가 잘린 상태로 끝나지 않도록 여러 장으로 나누고, 오른쪽 스와이프로 이어 본다.
- 글 상세의 관련 글은 별도 작은 리스트가 아니라 홈 피드 카드와 같은 형식으로 보여준다.
- 댓글은 기본으로 펼쳐두지 않고 사용자가 댓글 아이콘을 누르면 글쓰기 화면처럼 아래에서 위로 올라오는 단일 시트로 연다.
- 댓글 보기와 댓글 작성은 별도 창으로 나누지 않고 같은 시트 안에서 처리한다.
- 댓글 작성 권한이 없으면 설명 칩을 별도로 노출하지 않고 단일 댓글 시트 안의 작성 영역을 숨기거나 비활성화한다.
- 댓글 새로고침 텍스트 버튼은 제거하고, 위로 당겨 새로고침하는 방식으로 맞춘다.
- 내 글 수정/삭제는 본문 아래 관리 영역이 아니라 우상단 점 3개 메뉴에서 제공한다.
- 홈 피드 카드의 본문 캡션은 제거하고 이미지, 작가명, 반응 중심으로 단순화한다.
- 홈 피드 카드의 원형 프로필 마커는 제거하고, 대표 뱃지는 작가명 옆 작은 표시로만 둔다.
- 홈 피드 게시물 점 3개 메뉴에서는 `게시글 신고`, `작성자 차단`, `닫기`만 남긴다.
